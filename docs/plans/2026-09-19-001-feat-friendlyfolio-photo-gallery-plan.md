...
- R16. Every photo offers a download of the original file, with a meaningful filename.
...
- KTD9. **Every derivative is pre-generated; none is ever built in a request — and the bulk bake does not go through the job queue.** `ActiveStorage::Representations::BaseController#set_representation` calls `.processed` in a `before_action`, and both the proxy and redirect controllers inherit it, so a variant that does not yet exist is generated *synchronously inside the Puma request*. On a cold 100-tile wall that is 100 concurrent libvips invocations inside the web workers. Two different mechanisms follow, and conflating them is the trap: variants are declared `preprocessed: true` for the *ongoing* case, where an admin adds a photo or two and a `TransformJob` per variant is fine; the *initial* bake calls `.processed` directly from the rake task under its own worker pool. Declaring `preprocessed: true` and then ingesting 1,916 photos would enqueue ~17,250 `TransformJob`s into Solid Queue on SQLite within minutes, contending with the ingest's own inserts. R24 and AE3 exist to hold this line.
...
- Full-corpus pre-generation is estimated at 25 minutes to 2 hours of saturated CPU depending on core count, dominated by AVIF encoding. It runs once per edition, on a laptop rather than the server (KTD15), and is restartable — so its wall-clock is an operational note rather than a design constraint. The estimate is extrapolated from typical libvips behaviour, not measured on these photos; U3 measures a sample before the full run.
...
6. **U10's rake task is the ingest path; Avo is the curation path.** Avo Community's bulk multi-attachment behaviour is unverified, and the paid `avo-advanced_file_uploads` add-on exists precisely in that space — so do not design the 1,916-photo workflow around a browser upload. Probe what Community's file field does with a multi-file selection early, and if it handles tens of photos comfortably, keep it for the occasional addition. If it does not, say so and leave ingest entirely to the rake task rather than buying an add-on to solve a problem a rake task already solves.
...
- `photos:verify` reports a photo whose variant file has been deleted from disk.
...
3. Confirm the long-lived cache headers Rails already emits on representation URLs reach the response unmodified (KTD6). This is verification of an existing default, not new configuration. If the emitted directive lacks `immutable`, add it through Hatchbox's per-app Caddy custom headers rather than patching the controller.
